...
// Usar:
data = _.orderBy(data, d => d.TOTAL / d.POBLACION);

// Ordenar por region
data = _.orderBy(data, d => d.REGION);
...
svg
	...
	.attr('height', 14)
	.attr('y', function(d) {
		return y(d['DEPARTAMENTO']) - 7;
	})
	.attr('width', function(d) {
		return x(get_percent(d));
	})
	...
	.on('mouseover', function(d) {
		...
		div
			.html(
				'<div>Región: <span style="color:' +
					regionColors[d['REGION']] +
					'">' +
					d['REGION'] +
					'</span></div><div>Total niños: ' +
					d['POBLACION'] +
					'</div><div>Total computadores: ' +
					d['TOTAL'] +
					'</div>'
			)
...
svg.append('text').attr('transform', 'translate(500, 0)');
